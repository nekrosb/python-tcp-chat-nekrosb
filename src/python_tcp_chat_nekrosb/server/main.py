...
def main():
    ...
    console_thread = threading.Thread(
        target=console_input, args=(stop_event,), daemon=True
    )
    ...
    try:
        ...
        print(
            f"[bold red]Server shutting down.[/bold red] from keyboard interrupt at [yellow]{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}[/yellow]"
        )
    finally:
        server.close()
...
def console_input(stop_event):
    while not stop_event.is_set():
        ...
        if command.lower().strip() == "/exit":
            stop_event.set()
            logging.info("Server shutting down correctly from console input.")
            print(
                f"[bold red]Server shutting down correctly.[/bold red] from console input at [yellow]{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}[/yellow]"
            )
            break
